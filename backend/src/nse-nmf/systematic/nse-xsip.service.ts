...
@Injectable()
export class NseXsipService {
  private readonly logger = new Logger(NseXsipService.name)
  ...
  async registerXsip(advisorId: string, data: {
    clientId: string
    schemeCode: string
    schemeName?: string
    amount: number
    ...
    const registration = await this.prisma.nseSystematicRegistration.create({
      data: {
        ...
        stepUpAmount: data.stepUpAmount,
        stepUpPercent: data.stepUpPercent,
    ...
    if (isMockMode) {
      const mockResponse = this.mockService.mockXsipRegistrationResponse()
      ...
      this.auditLogService.log({
        ...
        details: { schemeCode: data.schemeCode, amount: data.amount, frequency: data.frequencyType, mandateId: data.mandateId, clientId: data.clientId },
      })
    ...
    const requestBody = {
      ...
      xsip_mandate_id: data.mandateId,
      step_up_amount: data.stepUpAmount?.toString() || '',
      step_up_percentage: data.stepUpPercent?.toString() || '',
