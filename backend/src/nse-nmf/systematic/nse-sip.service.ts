...
export class NseSipService {
  ...
  async registerSipTopup(advisorId: string, data: {
    clientId: string
    sipRegId: string
    schemeCode: string
    schemeName?: string
    ...
    if (isMockMode) {
      ...
      this.auditLogService.log({
        userId: advisorId,
        action: 'CREATE',
        entityType: 'NseSipTopup',
        entityId: registration.id,
        details: { schemeCode: data.schemeCode, amount: data.amount, sipRegId: data.sipRegId, clientId: data.clientId },
      })
    ...
    const requestBody = {
      client_code: nseUcc?.clientCode || client.pan,
      sch_code: data.schemeCode,
      topup_amount: data.amount.toString(),
      frequency_type: data.frequencyType,
      start_date: data.startDate,
      end_date: data.endDate || '',
      installment_no: data.installments?.toString() || '',
      folio_no: data.folioNumber || '',
      reg_id: data.sipRegId,
      euin_number: '',
      euin_declaration: 'Y',
    }

    const response = await this.httpClient.jsonRequest(
      NSE_ENDPOINTS.SIP_TOPUP,
      requestBody,
      credentials,
      advisorId,
      'SIP_TOPUP_REGISTRATION',
    )
