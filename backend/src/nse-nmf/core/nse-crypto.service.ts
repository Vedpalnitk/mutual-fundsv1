...
@Injectable()
export class NseCryptoService {
  private readonly logger = new Logger(NseCryptoService.name)
  private readonly algorithm = 'aes-256-gcm'
  private readonly keyBuffer: Buffer

  constructor(private config: ConfigService) {
    const key = this.config.get<string>('nmf.encryptionKey')
    if (key) {
      this.keyBuffer = Buffer.from(key, 'base64')
    } else {
      this.keyBuffer = randomBytes(32)
      this.logger.warn(
        'NMF_ENCRYPTION_KEY not set — using ephemeral key. Encrypted NSE credentials will not survive restarts. Set it in production!',
      )
    }
  }
