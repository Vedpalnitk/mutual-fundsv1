 ...
 * Action Center Page
 *
 * Centralized view of items requiring advisor attention:
 * Failed SIPs, Pending Transactions, KYC Pending clients.
 * Uses Promise.allSettled() for parallel, fault-tolerant data fetching.
 */
...
import { sipsApi, transactionsApi, clientsApi, FAPaginatedResponse } from '@/services/api'
...
const ActionCenterPage = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      if (sipsResult.status === 'fulfilled') {
        setFailedSips(sipsResult.value.data)
      } else {
        console.error('Failed to fetch failed SIPs:', sipsResult.reason)
        setFailedSips([])
      }

      if (txnResult.status === 'fulfilled') {
        setPendingTransactions(txnResult.value.data)
      } else {
        console.error('Failed to fetch pending transactions:', txnResult.reason)
        setPendingTransactions([])
      }

      if (clientsResult.status === 'fulfilled') {
        setKycPendingClients(clientsResult.value.data)
      } else {
        console.error('Failed to fetch KYC pending clients:', clientsResult.reason)
        setKycPendingClients([])
      }
  ...
  const handleContactClient = (clientName: string) => {
    showNotification('success', `Opening contact flow for ${clientName}`)
  }
  ...
  return (
    <AdvisorLayout>
      <div className="space-y-6">
        ...
        {loading ? (
          <FALoadingState message="Loading action items..." />
        ) : allEmpty ? (
          /* All Caught Up State */
          <FACard padding="lg">
            <FAEmptyState
              icon={<CheckCircleIcon className="w-12 h-12" style={{ color: colors.success }} />}
              title="All caught up!"
              description="There are no pending action items right now. Great work!"
            />
          </FACard>
        ) : (
          <>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              >
                <AlertTriangleIcon className="w-4 h-4" style={{ color: colors.error }} />
                <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: colors.error }}>
                  Failed SIPs
                </span>
                <span
              ...
              >
                <ClockIcon className="w-4 h-4" style={{ color: colors.warning }} />
                <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: colors.warning }}>
            ...
            {failedSipCount > 0 && (
              <FACard padding="lg">
                <FASectionHeader
                  title="Failed SIPs"
                  action={
                    <FAChip color={colors.error} size="sm">
                      {failedSipCount} {failedSipCount === 1 ? 'item' : 'items'}
                    </FAChip>
                  }
                />
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-1 flex-wrap">
                            ...
                            <span className="text-xs" style={{ color: colors.textTertiary }}>
                              {sip.lastSipDate ? formatDate(sip.lastSipDate) : formatDate(sip.startDate)}
                            </span>
                          </div>
                        </div>
                        <FAButton
                          ...
                          onClick={() => handleContactClient(sip.clientName)}
                        >
                          Contact Client
                        </FAButton>
                      </div>
            ...
            {pendingTxnCount > 0 && (
              <FACard padding="lg">
                <FASectionHeader
                  ...
                  action={
                    <FAChip color={colors.warning} size="sm">
                      {pendingTxnCount} {pendingTxnCount === 1 ? 'item' : 'items'}
                    </FAChip>
                  }
                />
                <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between flex-wrap gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <FAChip color={colors.warning} size="xs">
                              {txn.type}
                            </FAChip>
                            <p className="text-sm font-semibold truncate" style={{ color: colors.textPrimary }}>
                              {txn.fundName}
            ...
            {kycPendingCount > 0 && (
              <FACard padding="lg">
                <FASectionHeader
                  title="KYC Pending"
                  action={
                    <FAChip color={colors.primary} size="sm">
                      {kycPendingCount} {kycPendingCount === 1 ? 'client' : 'clients'}
                    </FAChip>
                  }
                />
                <div className="space-y-3">
