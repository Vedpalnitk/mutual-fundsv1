...
interface NmfOrderPlacementModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess?: (order: any) => void
  defaultType?: 'purchase' | 'redeem' | 'switch'
  clientId?: string
  staffEuin?: string
}

type OrderType = 'purchase' | 'redeem' | 'switch'
...
export default function NmfOrderPlacementModal({
  isOpen,
  onClose,
  onSuccess,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      switch (orderType) {
        case 'purchase':
          response = await nmfApi.orders.purchase({
          ...
          break
        case 'redeem':
          response = await nmfApi.orders.redeem({
            clientId: clientId.trim(),
            schemeCode: selectedScheme.schemeCode,
            amount: parsedAmount,
            schemeName: selectedScheme.schemeName,
          ...
          break
        case 'switch':
          response = await nmfApi.orders.switchOrder({
            clientId: clientId.trim(),
            fromSchemeCode: selectedScheme.schemeCode,
            toSchemeCode: toScheme!.schemeCode,
            amount: parsedAmount,
            fromSchemeName: selectedScheme.schemeName,
            toSchemeName: toScheme!.schemeName,
            ...euinArn,
          })
          break
      }
        ...
        <div className="px-5 py-4 overflow-y-auto space-y-5" style={{ maxHeight: 'calc(90vh - 130px)' }}>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Enter NMF client ID"
          ...
          <div>
            ...
            <NmfSchemePicker
              ...
              placeholder={orderType === 'switch' ? 'Search source scheme...' : 'Search schemes...'}
            />
            {selectedScheme && (
              <div
